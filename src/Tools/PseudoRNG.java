...
public class PseudoRNG
{
	...
	/**
	 * Convenience constructor that simply calls <code>{@link #PseudoRNG(int, int) PseudoRNG}(n, 1)</code>.
	 * <p>
	 ...
	 * This method is normally used when calling the seed specifying <code>{@link #PseudoRNG(int, int) constructor}</code>.
	 * <p>
   ...
   */
	...
	/*public static class UnitTest {
		...
		private void checkPeriod(int n, String mode) {
			PseudoRNG lfsr;
			if (mode.equals("")) lfsr = new PseudoRNG(n);
			else if (mode.equals("makeSeedNext")) lfsr = new PseudoRNG(n, makeSeedNext(n));
			else if (mode.equals("makeSeedRandom")) lfsr = new PseudoRNG(n, makeSeedRandom(n));
